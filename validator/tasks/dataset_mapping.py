...
from validator.shared.constants import STANDARD_INPUT_COLUMN
...
from validator.shared.constants import STANDARD_SYSTEM_COLUMN
...
def _text_column_mapping(response: dict) -> dict[str, str]:
    return {
        standard_key: response[response_key]
        for standard_key, response_key in _TEXT_COLUMN_RESPONSE_FIELDS.items()
        if response.get(response_key)
    }


def _dpo_column_mapping(response: dict) -> dict[str, str]:
    mapping = {
    ...
    if response.get("field_system"):
        mapping["system"] = response["field_system"]
    return mapping


def _grpo_column_mapping(response: dict) -> dict[str, str]:
    return {"prompt": response.get("field_prompt", "prompt")}


def _environment_column_mapping(_response: dict) -> dict[str, str]:
    return {"prompt": "prompt"}


_COLUMN_MAPPING_BY_TASK_TYPE = {
    TaskType.INSTRUCTTEXTTASK: _text_column_mapping,
    TaskType.CHATTASK: _text_column_mapping,
    TaskType.DPOTASK: _dpo_column_mapping,
    TaskType.GRPOTASK: _grpo_column_mapping,
    TaskType.ENVIRONMENTTASK: _environment_column_mapping,
}
_TEMP_TASK_FIELD_BY_STANDARD_KEY = {
...
async def get_dataset_column_mapping(dataset_id: str, task_type: TaskType, keypair: Keypair) -> dict[str, str]:
    ...
    from validator.shared.constants import CONTENT_BASE_URL
    ...
    mapping_builder = _COLUMN_MAPPING_BY_TASK_TYPE.get(normalized_task_type)
    if mapping_builder is None:
        raise ValueError(f"Unsupported task type: {task_type}")
    mapping = mapping_builder(response)
    logger.info(f"{normalized_task_type.value} column mapping for {dataset_id}: response={response}, mapping={mapping}")
    return mapping
...
def _standardize_sample_for_task(sample: dict, task: AnyTextTypeRawTask) -> dict:
    if isinstance(task, InstructTextRawTask):
        return standardize_instruct_sample(sample, task)
    if isinstance(task, DpoRawTask):
        return standardize_dpo_sample(sample, task)
    if isinstance(task, GrpoRawTask):
        return standardize_grpo_sample(sample, task)
...
def standardize_samples(samples: list[dict], task: AnyTextTypeRawTask) -> list[dict]:
    ...
    for i, sample in enumerate(samples):
        try:
            ...
            result = _standardize_sample_for_task(processed_sample, task)
...
def create_temp_task_from_mapping(column_mapping: dict[str, str], task_type):
    ...
    """
    normalized_task_type = normalize_task_type(task_type)
    field_mapping = _TEMP_TASK_FIELD_BY_STANDARD_KEY.get(normalized_task_type)
    if field_mapping is None:
        return None

    temp_task_dict = {"task_type": normalized_task_type}
    for standard_key, task_field in field_mapping.items():
        if standard_key in column_mapping:
            temp_task_dict[task_field] = column_mapping[standard_key]
    ...
    if len(dataset_ids) > 1:
        ...
        for dataset_id in dataset_ids[1:]:
            try:
                ...
                if normalize_task_type(task.task_type) == TaskType.DPOTASK:
                    try:
                        from validator.infrastructure.content_service import call_content_service_fast
                        from validator.shared.constants import CONTENT_BASE_URL
                ...
                if temp_task is None:
                    raise ValueError(f"Unsupported task type for multi-dataset merge: {task.task_type}")
