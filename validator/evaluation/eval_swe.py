...
async def _start_swe_env_server(env_base_url: str) -> tuple[object | None, asyncio.Task | None]:
    ...
    async def evaluate_single_task(session: aiohttp.ClientSession, seed: int, task_id: int, task_idx: int) -> dict:
        ...
        try:
            ...
            async with session.post(
                ...
                latency = result.get("time_taken", time.time() - start_ts) if isinstance(result, dict) else time.time() - start_ts
                logger.info(
...
async def _run() -> None:
    ...
    try:
        ...
        num_tasks = int(os.getenv("ENV_EVAL_NUM_SEEDS", str(env_config.get("num_seeds", 10))))
        payload_extra = dict(env_config.get("eval_payload_extra", {}))
        task_timeout = int(os.getenv("SWE_EVAL_TASK_TIMEOUT", str(env_config.get("task_timeout", 1800))))
        max_concurrency = int(
            os.getenv(
                "SWE_ENV_EVAL_MAX_CONCURRENT_REQUESTS",
                str(env_config.get("max_concurrent_requests", 1)),
            )
        )
