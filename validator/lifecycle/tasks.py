...
_MODEL_SIZE_MULTIPLIER_BY_TASK_TYPE = {
    TaskType.DPOTASK: 2,
    TaskType.GRPOTASK: 3,
    TaskType.ENVIRONMENTTASK: 3,
}


def _uses_batched_evaluation(task: AnyTypeRawTask) -> bool:
    return normalize_task_type(task.task_type) == TaskType.GRPOTASK or should_use_pvp(task)


def _resets_all_evaluation_rows(task: AnyTypeRawTask) -> bool:
    return normalize_task_type(task.task_type) == TaskType.GRPOTASK


def _model_size_multiplier(task_type: TaskType | str) -> int:
    return _MODEL_SIZE_MULTIPLIER_BY_TASK_TYPE.get(normalize_task_type(task_type), 1)
    ...
    with LogContext(task_id=str(task.task_id)):
        try:
            ...
            if not (task.is_organic and not cst.BASELINE_STATS_ENABLED_ORGANIC):
                ...
                logger.info(f"Task {task.task_id} prepped, moving to {task.status.value}")
...
async def _evaluate_pending_pairs_for_task(task: AnyTypeRawTask, num_gpus: int, config: Config):
    ...
    batch_together = _uses_batched_evaluation(task)
    if batch_together:
        training_statuses = await tournament_sql.get_training_status_for_task(str(task.task_id), config.psql_db)
        if training_statuses and any(status not in ("success", "failure") for status in training_statuses.values()):
...
async def _recover_evaluating_tasks(config: Config):
    ...
    for task in stopped_mid_evaluation:
        if task.task_id is None:
            continue
        if _resets_all_evaluation_rows(task):
...
def compute_required_gpus(task: RawTask) -> int:
    ...
    num_params = num_params * _model_size_multiplier(task.task_type)
