...
class AsyncMinioClient:
    ...
    async def ensure_bucket_exists(self, bucket_name):
        loop = asyncio.get_event_loop()
        exists = await loop.run_in_executor(self.executor, self.client.bucket_exists, bucket_name)
        if not exists:
            await loop.run_in_executor(self.executor, self.client.make_bucket, bucket_name)
