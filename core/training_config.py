...
def create_dataset_entry(
    ...
    dataset_entry.update(_process_dataset_fields(dataset_type))
...
def _process_dataset_fields(dataset_type: TextDatasetType) -> dict:
    task_type = task_type_for_dataset_type(dataset_type)

    if task_type == TaskType.INSTRUCTTEXTTASK:
        instruct_type_dict = {
            key: value for key, value in dataset_type.model_dump().items()
            if value is not None
        }
        return _process_instruct_dataset_fields(instruct_type_dict)

    processors = {
        TaskType.ENVIRONMENTTASK: _process_environment_dataset_fields,
        TaskType.DPOTASK: _process_dpo_dataset_fields,
        TaskType.GRPOTASK: _process_grpo_dataset_fields,
        TaskType.CHATTASK: _process_chat_template_dataset_fields,
    }

    try:
        return processors[task_type](dataset_type)
    except KeyError as exc:
        raise ValueError("Invalid dataset_type provided.") from exc
