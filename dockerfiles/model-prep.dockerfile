...
RUN pip install --no-cache-dir safetensors huggingface_hub numpy requests pydantic transformers datasketch aiohttp sglang python-dotenv basilica-sdk

COPY trainer/model_prep/ trainer/model_prep/
COPY core/ core/

# Validator modules needed for env stats + constants
