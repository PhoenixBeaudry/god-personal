...
@pytest.mark.asyncio
async def test_run_evaluation_docker_text_routes_swe_to_local_swe(monkeypatch):
    sentinel = object()
    captured = {}

    async def fake_swe_runner(
        models,
        original_model,
        dataset_type,
        file_format,
    ...
    async def fake_environment_runner(*args, **kwargs):
        raise AssertionError("SWE should not use the generic local environment runner")

    async def fake_intercode_runner(*args, **kwargs):
        raise AssertionError("SWE should not use the InterCode runner")

    monkeypatch.setattr(local_evaluation, "run_evaluation_local_swe", fake_swe_runner)
    monkeypatch.setattr(local_evaluation, "run_evaluation_local_environment", fake_environment_runner)
    monkeypatch.setattr(local_evaluation, "run_evaluation_local_intercode", fake_intercode_runner)
