...
class TestDetectLoraNames:
    def test_all_lora(self):
        ...
        with patch("validator.evaluation.pvp.group.check_for_lora", return_value=True):
            from validator.evaluation.pvp.group import _detect_lora_names
            names = _detect_lora_names(models)
    ...
    def test_mixed_lora_and_full_weight(self):
        ...
        with patch("validator.evaluation.pvp.group.check_for_lora", side_effect=side_effects):
            from validator.evaluation.pvp.group import _detect_lora_names
            names = _detect_lora_names(models)
...
class TestFullWeightExclusion:
    def test_full_weight_models_flagged(self):
        ...
        with patch("validator.evaluation.pvp.group.check_for_lora", side_effect=[True, False, True]):
            names = _detect_lora_names(models)
