...
class TestInstructStats:
    def test_returns_correct_type(self, model_and_tokenizer, instruct_data):
        ...
        stats = compute_text_stats(model, tok, instruct_data, task_type=TaskType.INSTRUCTTEXTTASK, max_samples=MAX_SAMPLES)
...
class TestDpoStats:
    def test_returns_correct_type(self, model_and_tokenizer, dpo_data):
        model, tok = model_and_tokenizer
        stats = compute_text_stats(model, tok, dpo_data, task_type=TaskType.DPOTASK, max_samples=MAX_SAMPLES)
...
class TestGrpoStats:
    def test_returns_correct_type(self, model_and_tokenizer, grpo_data):
        model, tok = model_and_tokenizer
        stats = compute_text_stats(model, tok, grpo_data, task_type=TaskType.GRPOTASK, max_samples=MAX_SAMPLES)
...
'''
        ...
        stats = compute_text_stats(
            model, tok, grpo_data, task_type=TaskType.GRPOTASK,
...
class TestChatStats:
    def test_returns_instruct_type(self, model_and_tokenizer, chat_data):
        ...
        model, tok = model_and_tokenizer
        stats = compute_text_stats(model, tok, chat_data, task_type=TaskType.CHATTASK, max_samples=MAX_SAMPLES)
        assert isinstance(stats, InstructBaselineStats)
