"""Tests for the PvP ↔ scoring integration layer:
should_use_pvp gate, environment ranking direction, batching.
"""

import math
from unittest.mock import MagicMock

from core.constants import EnvironmentName, EvalType, ENVIRONMENT_CONFIGS
from core.models.utility_models import TaskType
from validator.evaluation.scoring import should_use_pvp, calculate_miner_ranking_and_scores
from validator.core.models import MinerResultsText


# --- 5a: should_use_pvp gate ---


class TestShouldUsePvp:
    ...
    def test_all_configured_envs_are_pvp(self):
        """Every environment in ENVIRONMENT_CONFIGS with PVP eval_type triggers PvP."""
        for env_name, config in ENVIRONMENT_CONFIGS.items():
            if config.eval_type == EvalType.PVP:
                task = MagicMock()
                task.task_type = TaskType.ENVIRONMENTTASK
                task.environment_names = [env_name]
                assert should_use_pvp(task) is True, f"{env_name} should trigger PvP"
