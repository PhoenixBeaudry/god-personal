...
from validator.evaluation.tournament_scoring import get_tournament_weights_by_type
...
from validator.tournament.champions import get_real_winner_hotkey
...
class TestCalculateTournamentTypeScores:
    ...
    def test_weight_helpers_preserve_legacy_tuple_order(self):
        text_data = self._make_tournament_data(
            rounds=[
                TournamentRoundResult(
                    round_id="r1", round_number=1, round_type="group", is_final_round=False,
                    tasks=[
                        TournamentTaskScore(task_id="t1", group_id=None, pair_id=None, winner=BOB, participant_scores=[]),
                    ],
                ),
            ],
            winner_hotkey=ALICE,
        )
        environment_data = self._make_tournament_data(
            rounds=[
                TournamentRoundResult(
                    round_id="r1", round_number=1, round_type="group", is_final_round=False,
                    tasks=[
        ...
        weights_by_type = get_tournament_weights_by_type(
            {
                TournamentType.TEXT: text_data,
                TournamentType.IMAGE: None,
                TournamentType.ENVIRONMENT: environment_data,
            }
        )
        legacy_tuple = get_tournament_weights_from_data(text_data, None, environment_data)
