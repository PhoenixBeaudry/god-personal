...
from validator.shared.models import PeriodScore
from validator.shared.weight_setting import apply_tournament_weights
from validator.shared.weight_setting import get_node_weights_from_tournament_audit_data
...
class TestSeparatedBurnDynamics:
    ...
    def test_apply_tournament_weights(self, sample_tournament_participation):
        ...
        # Setup
        text_tournament_weights = {"hotkey1": 0.5, "hotkey3": 0.7}
        image_tournament_weights = {"hotkey2": 0.3, "hotkey3": 0.7}
        ...
        apply_tournament_weights(
            ...
            scaled_environment_tournament_weight=0.0,
            scaled_text_base_weight=0.05,
            scaled_image_base_weight=0.05,
            scaled_environment_base_weight=0.0,
            text_winner_hotkey="hotkey3",
            image_winner_hotkey="hotkey3",
            environment_winner_hotkey=None,
        )
    ...
    async def test_separated_burn_calculation_logic(self, mock_psql_db):
        ...
        with (
            patch("validator.shared.weight_setting.get_latest_completed_tournament") as mock_get_tournament,
            patch("validator.shared.weight_setting.calculate_performance_difference") as mock_calc_perf,
            patch("validator.shared.weight_setting._get_champion_decay") as mock_champion_decay,
        ):
            ...
            mock_environment_tournament = TournamentData(
                tournament_id="env_789",
                tournament_type=TournamentType.ENVIRONMENT,
                status="completed",
                base_winner_hotkey="winner3",
                winner_hotkey="winner3",
            )

            def mock_get_tournament_side_effect(psql_db, tournament_type, exclude_tournament_id=None):
                if exclude_tournament_id:
                    return TournamentData(
                        tournament_id=f"previous_{tournament_type.value}",
                        tournament_type=tournament_type,
                        status="completed",
                        base_winner_hotkey="previous_winner",
                        winner_hotkey="previous_winner",
                    )
                ...
                return mock_environment_tournament

            mock_get_tournament.side_effect = mock_get_tournament_side_effect
            mock_champion_decay.return_value = (0.0, 0.0, False)
            ...
            assert result.text_tournament_weight > result.image_tournament_weight
    ...
    async def test_integration_separated_weight_calculation(self, mock_psql_db):
        ...
        with (
            patch("validator.shared.weight_setting.fetch_nodes") as mock_fetch_nodes,
            patch("validator.shared.weight_setting.get_tournament_burn_details") as mock_burn_data,
            patch("validator.shared.weight_setting.get_weekly_task_participation_data") as mock_weekly_part,
            patch("validator.shared.weight_setting.get_active_tournament_participants") as mock_participants,
        ):
