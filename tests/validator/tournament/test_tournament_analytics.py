...
def mock_tournament_scoring():
    with patch('validator.endpoints.tournament_analytics.calculate_tournament_type_scores_from_data') as mock:
...
@pytest.mark.asyncio
async def test_tournament_with_group_and_final_rounds(
    mock_config,
    mock_tournament_sql,
    mock_task_sql,
    mock_tournament_scoring,
    mock_constants,
):
    ...
    assert not group_round.is_final_round
    ...
    assert final_round.is_final_round
    ...
    assert result.text_tournament_weight == get_tournament_spec(TournamentType.TEXT).base_weight
    assert result.image_tournament_weight == get_tournament_spec(TournamentType.IMAGE).base_weight
    ...
    async def manual_test():
        ...
        with patch('validator.endpoints.tournament_analytics.tournament_sql') as mock_sql, \
            ...
            try:
                ...
                print("✅ Tournament Details Test Passed!")
