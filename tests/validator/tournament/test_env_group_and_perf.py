...
class TestEnvGroupFormation:
    ...
    def test_max_plus_two_participants_split_balanced(self):
        """A group larger than MAX splits into balanced valid groups."""
    ...
    def test_12_participants_three_groups(self):
        """12 → 3 groups at the configured max size."""
        nodes = self._make_nodes(12)
        result = self._form_groups(nodes)
        assert len(result.groups) == 3
        assert all(len(g.member_ids) == t_cst.MAX_ENVIRONMENT_GROUP_SIZE for g in result.groups)

    def test_13_participants_three_groups(self):
        """13 → 4 groups (ceil(13/MAX)=4), roughly balanced."""
...
class TestEnvGroupWinnerAdvancement:
    ...
    async def test_top_n_advance_from_group(self):
        ...
        with (
            patch("validator.tournament.environment_results.get_tournament_group_members", return_value=mock_participants),
            patch("validator.tournament.environment_results.get_task_results_for_ranking", return_value=mock_results),
        ):
            config = MagicMock()
            psql_db = MagicMock()
    ...
    async def test_boss_excluded_from_advancement_ranking(self):
        """When the boss wins a single group, no challenger advances."""
        from validator.tournament.environment_results import get_environment_group_winners
    ...
    async def test_final_round_returns_all_participants(self):
        ...
        with patch("validator.tournament.environment_results.get_tournament_group_members", return_value=mock_participants):
            winners = await get_environment_group_winners(round_data, tasks, MagicMock(), MagicMock())
...
class TestDetermineEnvTournamentWinner:
    ...
    async def test_contender_wins_all_three(self):
        ...
        with (
            patch("validator.tournament.environment_results.get_tournament_rounds", return_value=mock_rounds),
            patch("validator.tournament.environment_results.get_tournament_tasks", return_value=mock_tasks),
            patch("validator.tournament.environment_results.get_scores_for_task", side_effect=mock_get_scores),
        ):
    ...
    async def test_no_final_round_boss_wins_default(self):
        ...
        # No rounds at all
        with patch("validator.tournament.environment_results.get_tournament_rounds", return_value=[]):
            result = await determine_env_tournament_winner(tournament, [], MagicMock(), MagicMock())
