...
COPY ops/docker/patches/axolotl_grpo_rollout_fix.py /workspace/axolotl/src/axolotl/core/trainers/grpo/__init__.py
COPY ops/docker/environment_functions/ /workspace/axolotl/src
COPY core /workspace/core
COPY trainer /workspace/trainer
COPY core/training_templates/base.yml /workspace/axolotl/base.yml
COPY core/training_templates/base_grpo.yml /workspace/axolotl/base_grpo.yml
COPY core/training_templates/base_environment.yml /workspace/axolotl/base_environment.yml

RUN chmod +x /workspace/trainer/containers/run_text_trainer.sh /workspace/trainer/containers/text_trainer.py

ENTRYPOINT ["/workspace/trainer/containers/run_text_trainer.sh"]
