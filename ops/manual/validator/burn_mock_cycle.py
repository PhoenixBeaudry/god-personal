"""
Manual probe to demonstrate tournament burn mechanics with mock data.
This creates mock tournaments and nodes to show how the burn/emission system works.
"""
...
LATEST_COMPLETED_TOURNAMENT_FN = "validator.shared.weight_setting.get_latest_completed_tournament"
...
async def scenario_1_perfect_performance():
    ...
    with patch(LATEST_COMPLETED_TOURNAMENT_FN, side_effect=[text_tournament, image_tournament]):
        with patch(LATEST_COMPLETED_TOURNAMENT_FN, side_effect=[prev_text, prev_image]):
...
async def scenario_5_only_text_tournament():
    ...
    with patch(
        "validator.shared.weight_setting.get_latest_completed_tournament", side_effect=[text_tournament, None]
    ):  # None for IMAGE
        with patch(LATEST_COMPLETED_TOURNAMENT_FN, return_value=prev_text):
            burn_data = await get_tournament_burn_details(mock_db)
...
def print_constants():
    ...
    print(
        f"   MAX_TEXT_TOURNAMENT_WEIGHT:    {cts.MAX_TEXT_TOURNAMENT_WEIGHT:.2f} "
        f"({cts.MAX_TEXT_TOURNAMENT_WEIGHT * 100:.0f}%)"
    )
    print(
