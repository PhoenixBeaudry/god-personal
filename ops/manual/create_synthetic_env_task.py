#!/usr/bin/env python3
"""Manual probe to create a synthetic environment task standalone."""
import asyncio
import os
import sys
import traceback
...
async def check_db_connection(config):
    """Check database connectivity before creating task."""
    try:
...
async def create_env_task():
    ...
    if not await check_db_connection(config):
