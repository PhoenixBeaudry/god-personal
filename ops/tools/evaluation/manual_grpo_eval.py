...
Usage: python -m ops.tools.evaluation.manual_grpo_eval <task_id> <model_repo>
"""
import asyncio
import os
import subprocess
    ...
    try:
        ...
        if os.path.exists(original_model_path):
            ...
            if tokenizer_files:
                ...
                print("⚠️ WARNING: No tokenizer files found in original model download!")
    except Exception as e:
...
async def main():
    if len(sys.argv) != 3:
        print("Usage: python -m ops.tools.evaluation.manual_grpo_eval <task_id> <model_repo>")
        print(
            "Example: python -m ops.tools.evaluation.manual_grpo_eval "
            "ba444f8a-dd60-40e2-bdad-524e1e754457 microsoft/Phi-3-mini-128k-instruct"
        )
        sys.exit(1)
