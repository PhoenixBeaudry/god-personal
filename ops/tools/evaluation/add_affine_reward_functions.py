...
from validator.tasks.affine_reward_functions import abd_reward_function
from validator.tasks.affine_reward_functions import ded_reward_function
from validator.tasks.affine_reward_functions import sat_reward_function
    ...
    try:
        async with pool.acquire() as conn:
            ...
            for func_name in function_names:
                ...
                await conn.execute(query, pattern)
...
async def main():
    ...
    if len(added_ids) == 3:
        ...
        constants_path = "validator/shared/constants.py"
    ...
    "{added_ids[2]}",  # ded_reward_function
        ...
        print("\n⚠️  NEXT STEPS:")
        print("1. Restart your validator to pick up the new constants")
        print("2. Check validator logs for 'Looking for affine reward functions' messages")
        print("3. Verify all 3 functions are found and loaded")
    else:
        print(f"\n❌ ERROR: Expected 3 function IDs, got {len(added_ids)}")
        print("📝 Current constants in validator/shared/constants.py:")
        print(f"AFFINE_REWARD_FN_IDS = {cst.AFFINE_REWARD_FN_IDS}")
        print("\n⚠️  MANUAL STEPS REQUIRED:")
        print("1. Check API responses above for any errors")
        print("2. Manually update AFFINE_REWARD_FN_IDS if needed")
        print("3. Restart your validator to pick up the new constants")
