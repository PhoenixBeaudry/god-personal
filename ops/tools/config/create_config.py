...
from ops.tools.config.input_validation import InputValidators
from ops.tools.config.input_validation import validate_input
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--miner", action="store_true", help="Deprecated: local miner runtime has been removed")
...
def generate_trainer_config() -> dict[str, Any]:
    ...
    config = TrainerConfig(
        ...
        huggingface_username=input("Enter your huggingface username where you would like to save the models: "),
    )
...
def generate_validator_config(dev: bool = False) -> dict[str, Any]:
    ...
    database_url = input("If using a remote database, enter the full database url (default: None): ") or None
    if database_url:
...
def generate_config(dev: bool = False, miner: bool = False, trainer: bool = False) -> dict[str, Any]:
    if miner:
        raise ValueError(
            "Local miner config generation has been removed. See docs/guide.md for the external miner contract."
        )
    elif trainer:
