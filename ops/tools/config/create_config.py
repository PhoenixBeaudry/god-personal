...
def generate_miner_config(dev: bool = False) -> dict[str, Any]:
    ...
    config = MinerConfig(
        ...
        miner_training_repo=validate_input("📦 Enter training repo URL: ", InputValidators.http_url),
        miner_training_commit=input("🔖 Enter training commit hash or branch (default: main): ") or "main",
        miner_github_token=input("🔐 Enter GitHub token for private repos (optional): ") or None,
        miner_requested_datasets=input("🤗 Enter requested datasets, comma-separated (optional): ") or None,
    )
