...
def get_task_type(request: TrainerProxyRequest) -> TaskType:
    ...
    if isinstance(training_data, TrainRequestText):
        return task_type_for_dataset_type(training_data.dataset_type)
...
async def start_training_task(task: TrainerProxyRequest, local_repo_path: str):
    ...
    try:
        ...
        is_image_training = is_image_task(task_type)
        is_environment_training = is_environment_task(task_type)
        ...
        download_status, exc = await asyncio.to_thread(
            ...
            task_id=training_data.task_id,
            model=training_data.model,
            dataset_url=training_data.dataset_zip if is_image_training else training_data.dataset,
            task_type=task_type,
            hotkey=task.hotkey,
            file_format=getattr(training_data, "file_format", None),
            model_type=training_data.model_type if is_image_training else None,
            log_labels=log_labels,
        ...
        tag, exc = await asyncio.to_thread(
            ...
            log_labels=log_labels,
            is_image_task=is_image_training,
        ...
        async def _final_cleanup():
            ...
            if success:
                try:
                    path_in_repo = cst.IMAGE_TASKS_HF_SUBFOLDER_PATH if is_image_training else None
                    wandb_token = os.getenv("WANDB_TOKEN") if not is_image_training else None
